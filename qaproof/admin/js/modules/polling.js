(function () {
  ...
  // NOTE: test history is no longer saved from the browser. The API persists
  // it server-side when the async job completes (single writer), so the old
  // `saveTestHistory` client POST — which produced a duplicate row per test —
  // has been removed. Callers just refresh the history list once the job is
  // done; see the onScreenshotsDone handlers in form.js / init.js.
