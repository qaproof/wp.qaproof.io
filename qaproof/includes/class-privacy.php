...
class QAProof_Privacy {
    ...
    public static function register_policy_content() {
        ...
        $content = wp_kses_post( sprintf(
            ...
            __(
                '<p>This site uses QAProof to run automated design and accessibility tests against its public pages. When you trigger a test from the WordPress admin (or when a scheduled monitor runs), the plugin sends the page URL, optional design source (Figma URL), and your configured QAProof API key to the QAProof API at api.qaproof.io. One request carries no API key: the first accessibility check, which you start from the QAProof dashboard before connecting an account. It sends only this site&#39;s public home page URL, and only when you click the button. The QAProof API renders the page in a headless browser, calls the Anthropic Claude Vision model to analyze the screenshots, and returns a structured report.</p>
                ...
                <p>When you submit a rating in the in-admin "How accurate was this analysis?" widget, the rating, optional comment, the test type, the tested page URL, the score, your WordPress user ID, and your site\'s home URL are sent to the QAProof API for product-quality analytics. Feedback is NOT stored on this WordPress site.</p>
                ...
                <p>Data stored locally by the plugin: the QAProof API key (non-autoloaded), the email address configured to receive regression notifications, and saved-design configurations (name and Figma URL). All test results, monitor definitions, monitor result history, visual regression baselines, and plugin feedback live on the QAProof SaaS, scoped to your QAProof workspace; this plugin does not create custom database tables for them on fresh installs.</p>
